...
export class ContactsService {
  ...
  list() {
    return this.http.get<Contact[]>(this.API).pipe(
      take(1),
  ...
  private update(record: Contact) {
    return this.http.put(`${this.API}/${record._id}`, record).pipe(take(1));
  }

  private create(record: Contact) {
